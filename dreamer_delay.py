...
class Dreamer(tf.keras.Model):
    ...
    def build_model(self):
        ...
        self.actor = models_delay.ActorNetwork(self.act_dim)
        model_modules = [self.encoder, self.dynamics, self.decoder, self.reward]
        Optimizer = functools.partial(tools_delay.Adam, clip=self.c.grad_clip)
    ...
    def _train(self, data):
        ...
        with tf.GradientTape() as actor_tape:
            ...
            returns = tools_delay.lambda_return(
...
def load_dataset(directory, config):
    episode = next(tools_delay.load_episodes(directory, 1))
    types = {k: v.dtype for k, v in episode.items()}
    shapes = {k: (None,) + v.shape[1:] for k, v in episode.items()}
    generator = lambda: tools_delay.load_episodes(
